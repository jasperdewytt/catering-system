...
## Status: Submission Cleanup Complete — Operator Console Is Primary
...
**Maintain the submission-ready operator console while preparing deferred production hardening.**

The deterministic Python backend can write generated order runs end-to-end, and the website can request that narrow generation path without shelling out or reimplementing ordering rules. Communications persistence captures recipient snapshots, email preparation events, and safety-gated test-routed send attempts. The Next.js website is the primary operator interface and has replaced the old Streamlit workflow. The implemented allocation algorithm remains:
...
The current approved fixture run has no allocation issues and the web caterer-email page can show persisted snapshots as email-ready. The manually verified bridge path also handles fresh approved runs with missing communication rows by creating the first immutable snapshot from the website. Website order generation creates a new persisted run and supersedes prior `blocked`/`generated` runs; it does not delete approved or historical runs. The shell sidebar points operators to the next likely workflow step from stored operator state, using the route week when present and the active week otherwise. The menu setup view may surface stored offers on unavailable variants as blocking menu-readiness findings, which the web workflow can resolve by changing availability or saving a revised offer set with an audited reason. Building-only delivery locations and suspicious/free-webmail caterer addresses are documented as expected competition-data artefacts, not blockers. Communications preserve recipient snapshots, delivery-note content, and provider send metadata for audit. The current send path is intentionally test-routed through `PADEA_EMAIL_TEST_RECIPIENT_OVERRIDE`; real-recipient rollout and website-editable SMTP settings remain deferred.

## Up Next (in order)

1. **Real-recipient email rollout** after the test-recipient path is operator-confirmed and encrypted secret management is designed.
2. **Broader Python job bridge** for ingestion and validation only after job status, retries, and audit semantics are designed.
3. **Persisted full validation history** through `session_validation_findings` if needed beyond the submission readiness summary.
4. **Applied meal edits** through a backend/RPC contract that validates replacements and records before/after audit state.
5. **Optional advisory LLM layer** without changing deterministic ordering, dietary, quantity, attendance, or approval rules.
6. Remove deprecated Streamlit files and the unused dependency in a separate cleanup if no historical reference is needed.
