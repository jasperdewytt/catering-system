...
- [x] **V1 caterer email sending implemented** — FastAPI `POST /internal/caterer-email-sends` sends reviewed persisted snapshots through Python-owned Gmail SMTP configuration, requires `PADEA_EMAIL_TEST_RECIPIENT_OVERRIDE`, blocks v1 resends of `sent` communications, and records `sent`/`send_failed` communication events plus `communication_sent`/`communication_send_failed` audit rows. `/weeks/[weekStart]/exports` now exposes per-caterer send and send-all-ready controls without storing SMTP credentials in Next.js.
...
The deterministic Python backend can write generated order runs end-to-end, and the website can now request that narrow generation path without shelling out or reimplementing ordering rules. Communications persistence captures recipient snapshots, email preparation events, and v1 test-routed send attempts. The Next.js website is now the primary operator interface and has replaced the Streamlit MVP workflow. The implemented allocation algorithm remains:
...
4. Real-recipient email sending only after the v1 test-routed send workflow is operator-confirmed in `web/` and encrypted secret management is designed.
...
- real-recipient email sending — v1 send is implemented with a mandatory test-recipient override; production rollout remains deferred
