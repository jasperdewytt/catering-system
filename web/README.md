...
caterer email records, queue feedback dispatch, and submit public feedback
forms. Those server-side actions also need:
...
Signed student and session-manager feedback links are generated by the Python
backend with `PADEA_FEEDBACK_LINK_SECRET`; the website only displays or submits
those links through backend contracts. Public feedback pages never write to
Supabase directly.
...
- tokenized student/session-manager feedback forms and `/feedback` quality dashboard
